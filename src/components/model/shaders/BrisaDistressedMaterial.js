...
export const createBrisaDistressedMaterial = (fabricColor, stitchColor, textures, ambientStrength = 0.5, specularPower = 12.0, specularIntensity = 0.25, isTwoTone = false, noStitching = false, externalStitchColor = null) => {
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      // ✅ Enhanced fabric texture visibility for distressed leather
      vec3 grainSample = texture2D(grainMap, vUv * 24.0).rgb; // Slightly larger scale for distressed detail
      float fineGrain = dot(grainSample, vec3(0.299, 0.587, 0.114)); // Luminance for better contrast
      
      // Increase texture contrast for distressed pattern visibility
      fineGrain = pow(fineGrain, 0.65); // More contrast for distressed look
      fineGrain = fineGrain * 1.3 - 0.15; // Enhanced range
      fineGrain = clamp(fineGrain, 0.0, 1.0);
      
      // Blend procedural and texture grain - distressed needs high texture visibility
      grain = mix(grain, fineGrain, 0.9); // High texture contribution for distressed
      
      // Create natural distressed variation with enhanced texture visibility
      vec3 naturalLeather = baseLeatherColor * (0.7 + 0.3 * grain); // More variation for distressed
      
      // Add prominent distressed texture detail
      float darkness = 1.0 - grain;
      naturalLeather = mix(naturalLeather, baseLeatherColor * 0.6, darkness * 0.35); // Stronger distressed detail
      
      // Enhanced lighting with texture-based normal variation for distressed texture
      vec3 normal = normalize(vNormal);
      vec3 lightDir = normalize(vec3(1.0, 1.0, 1.0));
      vec3 viewDir = normalize(cameraPosition - vWorldPosition);
      
      vec3 grainNormal = texture2D(grainMap, vUv * 24.0).rgb;
      float grainNormalStrength = (grainNormal.r - 0.5) * 0.45; // Stronger normal for distressed
      ...
      float grainLighting = 0.75 + 0.25 * grain; // More texture influence for distressed
      ...
      // Enhanced specular with texture-based variation (distressed leather is matte but has texture)
      ...
      // Use grain texture to modulate specular for distressed appearance
      float grainSpecMod = texture2D(grainMap, vUv * 24.0).r;
