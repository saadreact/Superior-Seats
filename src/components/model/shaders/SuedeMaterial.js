 ...
 */
export const createSuedeMaterial = (fabricColor, stitchColor, textures, ambientStrength = 0.5, specularPower = 8.0, specularIntensity = 0.15, isTwoTone = false, noStitching = false, externalStitchColor = null) => {
  // ✅ Load fine-grain procedural bump texture for extra realism
  const leatherGrainTexture = new THREE.TextureLoader().load('/assets/fabrics/SuedeMaterial.png');
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      // ✅ Enhanced fabric texture visibility for suede
      vec3 grainSample = texture2D(grainMap, vUv * 22.0).rgb; // Slightly larger scale for suede nap
      float fineGrain = dot(grainSample, vec3(0.299, 0.587, 0.114)); // Luminance for better contrast
      
      // Increase texture contrast for suede nap visibility
      fineGrain = pow(fineGrain, 0.65); // More contrast for suede
      fineGrain = fineGrain * 1.3 - 0.15; // Enhanced range
      fineGrain = clamp(fineGrain, 0.0, 1.0);
      
      // Blend procedural and texture grain - suede needs high texture visibility
      grain = mix(grain, fineGrain, 0.9); // High texture contribution for suede
      
      // Create natural suede variation with enhanced texture visibility
      vec3 naturalLeather = baseLeatherColor * (0.7 + 0.3 * grain); // More variation for suede
      
      // Add prominent suede nap texture detail
      float darkness = 1.0 - grain;
      naturalLeather = mix(naturalLeather, baseLeatherColor * 0.6, darkness * 0.35); // Stronger nap detail
      
      // Enhanced lighting with texture-based normal variation for suede nap
      vec3 normal = normalize(vNormal);
      vec3 lightDir = normalize(vec3(1.0, 1.0, 1.0));
      vec3 viewDir = normalize(cameraPosition - vWorldPosition);
      
      vec3 grainNormal = texture2D(grainMap, vUv * 22.0).rgb;
      float grainNormalStrength = (grainNormal.r - 0.5) * 0.5; // Stronger normal for suede nap
      ...
      float grainLighting = 0.75 + 0.25 * grain; // More texture influence for suede
      ...
      // Enhanced specular with texture-based variation (suede is matte but has texture)
      ...
      // Use grain texture to modulate specular for suede nap appearance
      float grainSpecMod = texture2D(grainMap, vUv * 22.0).r;
      ...
      if (fabricLuminance < 0.2) {
        float darkBoost = 1.0 + (0.2 - fabricLuminance) * 2.5;
        specular *= darkBoost;
        specular = min(specular, vec3(1.5));
      }
      float fresnel = 1.0 - max(dot(viewDir, enhancedNormal), 0.0);
      fresnel = pow(fresnel, 2.0);
      vec3 rimLight = vec3(0.0);
      if (fabricLuminance < 0.2) {
        float rimIntensity = (0.2 - fabricLuminance) * 0.12;
      ...
      if (fabricLuminance < 0.2) {
        float textureBoost = 1.0 + (0.2 - fabricLuminance) * 0.4;
 ...
 */
export const updateSuedeUniforms = (material, fabricColor, stitchColor, ambientStrength, specularPower, specularIntensity, externalStitchColor = null) => {
