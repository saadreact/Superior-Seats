 ...
 */
export const createLeatherMaterial = (fabricColor, stitchColor, textures, ambientStrength = 0.5, specularPower = 20.0, specularIntensity = 0.4, isTwoTone = false, noStitching = false, externalStitchColor = null, lightDirection = [1.0, 1.0, 1.0]) => {
  ...
  if (textures.grainTexture) {
    ...
    // Fallback to procedural texture if no grain texture provided
    leatherGrainTexture = new THREE.TextureLoader().load('/assets/fabrics/PremiumLeather.png');
    leatherGrainTexture.wrapS = leatherGrainTexture.wrapT = THREE.RepeatWrapping;
    leatherGrainTexture.anisotropy = 16; // Increased from 8 to 16 for better texture quality
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      // ✅ Enhanced fabric texture visibility - blend in grain texture more prominently
      vec3 grainSample = texture2D(grainMap, vUv * 25.0).rgb; // Slightly larger scale for better visibility
      float fineGrain = dot(grainSample, vec3(0.299, 0.587, 0.114)); // Use luminance for better contrast
      
      // Increase texture contrast for better visibility
      fineGrain = pow(fineGrain, 0.7); // Increase contrast (lower = more contrast)
      fineGrain = fineGrain * 1.2 - 0.1; // Brighten and enhance range
      fineGrain = clamp(fineGrain, 0.0, 1.0);
      
      // Blend procedural and texture grain - increase texture contribution for prominence
      grain = mix(grain, fineGrain, 0.85); // Increased from 0.6 to 0.85 for more visible texture
      
      // Create natural leather variation with enhanced texture visibility
      vec3 naturalLeather = baseLeatherColor * (0.75 + 0.25 * grain); // Increased variation range
      
      // Add more prominent darker areas for depth (leather valleys) - enhanced texture detail
      float darkness = 1.0 - grain;
      naturalLeather = mix(naturalLeather, baseLeatherColor * 0.65, darkness * 0.25); // Increased from 0.15 to 0.25
      ...
      // Enhanced lighting with texture-based normal variation for better depth perception
      // Use grain texture to create subtle normal map effect
      vec3 grainNormal = texture2D(grainMap, vUv * 25.0).rgb;
      float grainNormalStrength = (grainNormal.r - 0.5) * 0.3; // Subtle normal variation
      vec3 enhancedNormal = normalize(normal + grainNormalStrength * normalize(vNormal));
      
      // Natural leather lighting with enhanced texture-based variation
      float NdotL = max(dot(enhancedNormal, lightDir), 0.0);
      float grainLighting = 0.8 + 0.2 * grain; // Increased grain influence for texture visibility
      vec3 diffuse = naturalLeather * NdotL * grainLighting * 0.9; // Increased from 0.8 to 0.9 for better visibility
      ...
      // DRAMATIC change: black gets 10% ambient (was 50%), dark gray gets 20%
      float adjustedAmbientStrength = fabricLuminance < 0.1 ? ambientStrength * 0.1 : 
                                      fabricLuminance < 0.2 ? ambientStrength * 0.2 : 
                                      ambientStrength;
      vec3 ambient = naturalLeather * adjustedAmbientStrength;
      
      // Enhanced specular with texture-based variation for better material definition
      vec3 reflectDir = reflect(-lightDir, enhancedNormal);
      float specPower = specularPower + 10.0 * grain; // Increased grain influence
      float spec = pow(max(dot(viewDir, reflectDir), 0.0), specPower);
      // Use grain texture to modulate specular for more realistic material appearance
      float grainSpecMod = texture2D(grainMap, vUv * 25.0).r;
      vec3 specular = vec3(specularIntensity) * spec * (0.65 + 0.35 * grain) * (0.9 + 0.1 * grainSpecMod);
      
      // ✅ DARK MATERIAL ENHANCEMENTS: Prevent flat black appearance
      // 1. Enhanced specular for dark materials (brighter, more visible highlights)
      if (fabricLuminance < 0.2) {
        float darkBoost = 1.0 + (0.2 - fabricLuminance) * 3.0; // Up to 3x brighter specular for black
        specular *= darkBoost;
        specular = min(specular, vec3(2.0)); // Clamp to prevent overexposure
      }
      
      // 2. Fresnel/Rim lighting for dark materials (edge glow effect)
      float fresnel = 1.0 - max(dot(viewDir, enhancedNormal), 0.0);
      fresnel = pow(fresnel, 2.0); // Sharper falloff
      vec3 rimLight = vec3(0.0);
      if (fabricLuminance < 0.2) {
        float rimIntensity = (0.2 - fabricLuminance) * 0.15; // Stronger rim for darker materials
        rimLight = vec3(rimIntensity) * fresnel * (0.3 + 0.7 * NdotL); // Rim light that respects lighting
      }
      
      // 3. Subtle color variation for black materials (real black has slight blue/brown tint)
      vec3 colorVariation = baseLeatherColor;
      if (fabricLuminance < 0.15) {
        // Add very subtle color variation based on grain
      ...
      if (fabricLuminance < 0.2) {
        // Increase texture contrast on dark materials
        float textureBoost = 1.0 + (0.2 - fabricLuminance) * 0.5; // Up to 1.5x texture visibility
      ...
      // DRAMATIC change: black gets 5% min shadow (was 50%), dark gray gets 10%
      float aoMin = fabricLuminance < 0.1 ? 0.05 : 
                    fabricLuminance < 0.2 ? 0.1 : 
                    0.4; // Lighter fabrics = softer shadows (40% min)
 ...
 */
export const updateLeatherUniforms = (material, fabricColor, stitchColor, ambientStrength, specularPower, specularIntensity, externalStitchColor = null, lightDirection = null) => {
