 ...
 */
export const createMeshMaterial = (fabricColor, stitchColor, textures, ambientStrength = 0.5, specularPower = 10.0, specularIntensity = 0.2, isTwoTone = false, noStitching = false, externalStitchColor = null) => {
  // ✅ Load fine-grain procedural bump texture for extra realism
  const leatherGrainTexture = new THREE.TextureLoader().load('/assets/fabrics/BreathableMesh.png');
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      // ✅ Enhanced fabric texture visibility for mesh weave
      vec3 grainSample = texture2D(grainMap, vUv * 18.0).rgb; // Larger scale for mesh weave visibility
      float fineGrain = dot(grainSample, vec3(0.299, 0.587, 0.114)); // Luminance for better contrast
      
      // Increase texture contrast for mesh weave visibility
      fineGrain = pow(fineGrain, 0.6); // More contrast for mesh
      fineGrain = fineGrain * 1.4 - 0.2; // Enhanced range
      fineGrain = clamp(fineGrain, 0.0, 1.0);
      
      // Blend procedural and texture grain - mesh needs very high texture visibility
      grain = mix(grain, fineGrain, 0.92); // Very high texture contribution for mesh
      
      // Create natural mesh variation with enhanced texture visibility
      vec3 naturalLeather = baseLeatherColor * (0.7 + 0.3 * grain); // More variation for mesh
      
      // Add prominent mesh weave texture detail
      float darkness = 1.0 - grain;
      naturalLeather = mix(naturalLeather, baseLeatherColor * 0.6, darkness * 0.4); // Stronger weave detail
      
      // Enhanced lighting with texture-based normal variation for mesh weave
      vec3 normal = normalize(vNormal);
      vec3 lightDir = normalize(vec3(1.0, 1.0, 1.0));
      vec3 viewDir = normalize(cameraPosition - vWorldPosition);
      
      vec3 grainNormal = texture2D(grainMap, vUv * 18.0).rgb;
      float grainNormalStrength = (grainNormal.r - 0.5) * 0.5; // Stronger normal for mesh
      ...
      float grainLighting = 0.75 + 0.25 * grain; // More texture influence for mesh
      ...
      // Enhanced specular with texture-based variation (mesh is matte but has texture)
      ...
      // Use grain texture to modulate specular for mesh weave appearance
      float grainSpecMod = texture2D(grainMap, vUv * 18.0).r;
 ...
 */
export const updateMeshUniforms = (material, fabricColor, stitchColor, ambientStrength, specularPower, specularIntensity, externalStitchColor = null) => {
