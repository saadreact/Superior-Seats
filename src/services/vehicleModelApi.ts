...
// Vehicle Model API service class
class VehicleModelApiService {
  ...
  async getVehicleModels(params: {
    ...
    try {
      ...
      const response = await api.get(`/vehicle-models?${queryString}`);
  ...
  async getVehicleModel(id: number) {
    try {
      const response = await api.get(`/vehicle-models/${id}`);
      
      // Handle the specific response structure: response.data.data.vehicle_model
      if (response.data && response.data.data && response.data.data.vehicle_model) {
        return response.data.data.vehicle_model;
      } else if (response.data && response.data.data) {
      ...
      throw new Error(error.response?.data?.message || 'Failed to fetch vehicle model');
    }
  }
  ...
  async createVehicleModel(data: {
    ...
    try {
      ...
      throw new Error(error.response?.data?.message || 'Failed to create vehicle model');
    }
  }

  // Update existing vehicle model
  async updateVehicleModel(id: number, data: {
    name?: string;
    description?: string;
    vehicle_make_id?: number;
    is_active?: boolean;
    ...
    try {
      ...
      throw new Error(error.response?.data?.message || 'Failed to update vehicle model');
    }
  }
  ...
  async deleteVehicleModel(id: number) {
    try {
      ...
      throw new Error(error.response?.data?.message || 'Failed to delete vehicle model');
    }
  }
}

// Export singleton instance
export const vehicleModelApiService = new VehicleModelApiService();
export default vehicleModelApiService;
